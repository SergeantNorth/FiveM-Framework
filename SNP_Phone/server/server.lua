...
if (GetCurrentResourceName() ~= "SNP_Phone") then
    print("[^1DEBUG^0] Please make sure the resource name is ^3SNP_Phone^0 or else exports won't work.")
end
...
AddEventHandler('gcPhone:playerLoaded',function(src)
    ...
    if(Config.enable_money_system == true) then
        local player = exports.SNP_Money:getaccount(sourcePlayer)
...
if(Config.enable_money_system == true) then
    ...
    AddEventHandler('gcPhone:CheckTranscrion', function(id, amount) 
        ...
        if(amount < 0 or amount == nil) then
            ...
            if(src == id) then
                exports.SNP_Money:bankNotify(src, "You cannot transfer money to your self.")
            else
                ...
                if(quickmath < 0 or quickmath == nil) then
                    ...
                    exports.SNP_Money:updateaccount(src, xarray)
                    exports.SNP_Money:updateaccount(id, zarray)
                    exports.SNP_Money:bankNotify(src, "You have transfered ~r~$" .. amount .. " to " .. GetPlayerName(tonumber(id)) .. " [#" .. id .. "]")
                    exports.SNP_Money:bankNotify(id, "You have received: ~g~$" .. amount .. " from ~g~" .. GetPlayerName(tonumber(src)) .. " [#" .. src .. "]")
                    local player = exports.SNP_Money:getaccount(id)
                    local shit = exports.SNP_Money:getaccount(src)
...
if(Config.enable_money_system == true) then
    ...
    AddEventHandler('gcPhone:UpdateBANKKKKK', function() 
        local src = source
        local player = exports.SNP_Money:getaccount(src)
