...
if (GetCurrentResourceName() ~= "SNP_Money") then
    print("[^1DEBUG^0] Please make sure the resource name is ^3SNP_Money^0 or else exports won't work.")
end
...
AddEventHandler('SERGEANTNORTH:CHECKSQL', function(steam, discord, first_name, last_name, twt, dept, dob, gender, data) 
    ...
    MySQL.Async.fetchAll("SELECT * FROM money WHERE steam = @steam AND first = @first AND last = @last AND dept = @dept", {["@steam"] = steam, ["@first"] = first_name, ["@last"] = last_name, ["@dept"] = dept}, function(data) 
        if(data[1] == nil) then
            ...
            TriggerClientEvent('SERGEANTNORTH:UPDATECLIENTMONEY', src, accounts[src])
        else 
...
--             TriggerClientEvent('SERGEANTNORTH:UPDATECLIENTMONEY', source, accounts[source])
--         end
--     end)
-- end
        ...
        for _, player in ipairs(GetPlayers()) do
            ...
            if(accounts[player]) then
                if(accounts[player].dept) then
                    ...
                    TriggerClientEvent('SERGEANTNORTH:UPDATEPAYCHECK', player, player, accounts[player])
                end
            end
        end
    end
...
exports('updateaccount', function(id, array) 
    if(accounts[id]) then
        ...
        TriggerClientEvent('SERGEANTNORTH:UPDATEPAY', id, id, accounts[id])
        return true
    else 
        return nil
    end
...
exports('sendmoney', function(id, sendid, idarray, sendidarray) 
    ...
    if(accounts[sendid]) then
        ...
        TriggerClientEvent('SERGEANTNORTH:UPDATEPAY', sendid, id, accounts[sendid])
    end
end)


exports('bankNotify', function(id, message) 
    TriggerClientEvent('SERGEANTNORTH:BANKNOTIFY', id, message)
end)