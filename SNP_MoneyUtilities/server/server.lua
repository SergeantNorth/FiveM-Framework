...
if (GetCurrentResourceName() ~= "SNP_MoneyUtilities") then
    print("[^1DEBUG^0] Please make sure the resource name is ^3SNP_MoneyUtilities^0 or else exports won't work.")
end
...
AddEventHandler('bank:deposit', function(amount)
	...
	local xPlayer = exports.SNP_Money:getaccount(_source)
	amount = tonumber(amount)
	if xPlayer == nil or amount == nil or amount < 0 then
		exports.SNP_Money:bankNotify(_source, "There was an error getting the account Information")
	else
		local quickmath = xPlayer.amount - amount;
		if(quickmath < 0 or quickmath == nil) then
			exports.SNP_Money:bankNotify(_source, "You don't have enough money to execute this transaction.")
		else 
			local fix = {cash = quickmath, bank = xPlayer.bank + amount}
			exports.SNP_Money:updateaccount(_source, fix)
			exports.SNP_Money:bankNotify(_source, "You have deposited: ~g~$" .. amount)
		end
	end
end)
...
AddEventHandler('bank:withdraw', function(amount)
	...
	if xPlayer == nil or amount == nil or amount < 0 then
		...
		if(quickmath < 0 or quickmath == nil) then
			...
			exports.SNP_Money:updateaccount(_source, fix)
			exports.SNP_Money:bankNotify(_source, "You have withdraw: ~g~$" .. amount)
		end
	end
end)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = exports.SNP_Money:getaccount(rip)
	local amount = tonumber(amountt)
	if tonumber(_source) == rip then
		exports.SNP_Money:bankNotify(_source, "You cannot transfer funds to yourself.")
	elseif (amount == nil or amount < 0) then
		exports.SNP_Money:bankNotify(_source, "Invalid amount.")
	else
		if (rip == nil) then
			exports.SNP_Money:bankNotify(_source, "You cannot transfer funds to yourself.")
		else 
			...
			if(quickmath < 0 or quickmath == nil) then
				...
				exports.SNP_Money:updateaccount(_source, xarray)
				exports.SNP_Money:updateaccount(rip, zarray)
				exports.SNP_Money:bankNotify(_source, "You have transfered ~r~$" .. amount .. " to " .. GetPlayerName(tonumber(to)) .. " [#" .. to .. "]")
				exports.SNP_Money:bankNotify(rip, "You have received: ~g~$" .. amount .. " from ~g~" .. GetPlayerName(tonumber(_source)) .. " [#" .. _source .. "]")
			end	
		end
	end
end)
		...
		if(id == source) then
			...
			if(name == nil) then
				...
				if(amount == nil or amount < 0) then
					...
					local xPlayer = exports.SNP_Money:getaccount(_source)
					local zPlayer = exports.SNP_Money:getaccount(id)
					...
					if(check < 0) then 
						...
						exports.SNP_Money:updateaccount(_source, xarray)
						exports.SNP_Money:updateaccount(id, zarray)
						exports.SNP_Money:bankNotify(id, "You have received: ~g~$" .. amount .. " from ~g~" .. GetPlayerName(tonumber(_source)) .. " [#" .. _source .. "]")
